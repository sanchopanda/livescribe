...
4. **Любая работа идёт через доску — сначала карточка, потом код.** Доска — **self-hosted
   Vikunja, https://kanban.skribo.ru**, проект **Skribo** (id 5); GitHub — только код/коммиты/
   релизы. Колонки: Входящие → Бэклог → В работе → На проверке → Готово. На каждую единицу
   работы (фича/баг/фикс, даже однострочный) заводится или берётся карточка и переводится в
   **«В работе»**, и только потом реализация; после реализации → **«На проверке»** с кратким
   комментарием что сделано, после деплоя → **«Готово»**. Метки: **feature** / **bug**. Всё это
   агент делает **сам, автономно, без вопросов пользователю** про доску. Механика (Vikunja REST
   API, креды `~/.skribo-vikunja.env`) — в скилле `task-board`; «взять следующую» — `check-tasks`;
   фоновый опрос новых карточек — `watch-board`.
5. **Найденное по ходу — своя карточка.** Дефект, всплывший при разборе другой задачи, получает
   отдельную карточку сразу, даже если чинить его в этом проходе не будем: иначе находка живёт
   только в диалоге и теряется вместе с сессией. `docs/backlog.md` остаётся планировочным
   документом, id `LS-NN` — общий ключ между ним и доской.
...
- **`task-board`** — механика доски Vikunja (проект Skribo id 5): чтение, перенос карточек,
  комментарии, метки.
- **`check-tasks`** — показать Входящие/Бэклог, спросить какую брать, провести по циклу.
  Триггеры: «проверь задачи / проверь доску / возьми задачу / что на доске».
- **`watch-board`** — фоновый опрос доски раз в ~30 мин (session-only, перезапускать после
  старта Claude Code).
