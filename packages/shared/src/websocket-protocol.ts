 ...
 *  - 'failed' — переподключиться не удалось (лимит попыток исчерпан): текст
 *    по этой сессии больше не появится, но встреча НЕ потеряна — аудио всё
 *    равно записано, расшифровку можно будет получить позже (повторной
 *    обработкой), поэтому это повод показать предупреждение, а не прервать
 *    запись со стороны клиента.
 */
