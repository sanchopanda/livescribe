...
- **Diarization**: Currently disabled in runtime (`diarize: false`)
- **Speaker Attribution**: Uses DOM speaker updates from extension/session
...
1. Current DOM speaker (`session.speaker`) is used.
2. If no DOM speaker is available, `speaker` may be omitted.

Experimental diarization mapping logic is archived in `docs/SPEAKER_DETECTION_ARCHIVE.md`.
