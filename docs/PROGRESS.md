...
- LS-09 под-проект **B** — live-саммари в расширении (быстрая модель `LLM_MODEL_LIVE`, эфемерно,
  кнопка в виджете): отдельные спека/план.
- Либо: задеплоить LS-09A на сервер (`OPENROUTER_API_KEY`) и проверить вживую; LS-12 (email/сброс пароля).
- Опц. follow-up LS-14: desktop-нотификация; предзаполнить триггеры именем аккаунта.

## Задача в работе

- Нет (LS-09A закрыт). Локальные коммиты не запушены в `origin`.
