...
## Сделано (последнее, Meet)

- **LS-33 — Meet подписывал все реплики владельцем записи.** Активный говорящий определялся по
  числу классов в индикаторе, а у self-плитки постоянный лишний класс `KUNJSe` — она выигрывала
  всегда. Теперь правило «нет idle-класса `gjg47c`», с молчанием при устаревшем маркере, плюс
  разбор актуального формата aria-label («{Имя}: ещё варианты»). Коммит `7459334`.
  ✅ Проверено на живом звонке: Даниил и Сергей подписываются верно.
- **LS-35 — в per-track писался только владелец записи.** Захват шёл по карте владельцев, а она
  пуста без `data-ssrc` (у Meet его нет) — чужие дорожки молча терялись. Захват развязан с
  именованием (`capture-targets.ts`, `4d901ed`), имена даёт корреляция «энергия дорожки ↔ подсветка
  плитки» с памятью (`speaker-binding.ts`, `88143bd`..`5fb251b`), уровни живут 400 мс
  (`track-energy.ts`, `41f4ecd`), переименование задним числом — `c403cbb`, `3576b1a`, `53a4463`,
  `83dcfca`, `7b38191`. Фильтр замьюченных отменён (`9b1ef0c`, причина — в `KNOWLEDGE.md`).
  ⚠️ Браузерная проверка per-track с именами — за владельцем.
- **LS-34 заведён:** в mixed своего голоса нет в потоке вообще (`chromeMediaSource: 'tab'`).
...
## Сделано (последнее, смок облачных STT для LS-13)

- **Смок-проверка облачных STT (Nemotron/Whisper через Together AI против nova-3).**
  Одноразовый скрипт `packages/backend/scripts/stt-smoke/` прогнал три провайдера на двух
  реальных звонках (1:56 и 6:14) в реальном темпе. Итог: nova-3 лучший по цельности/пунктуации,
  Whisper (с явным `language=ru`) близок, Nemotron содержательно верен но грязнее; Deepgram
  отдаёт текст каждые ~3.8 с против ~9–20 с у Together (управляемый VAD-компромисс, не свойство
  моделей); все три провайдера прошли 6-минутную сессию без разрывов. SaluteSpeech (GigaAM) вне
  объёма — облачного доступа к нему для физлица нет. **Решение владельца (2026-08-07): остаёмся
  на Deepgram, пока действует триал** — экономика на сейчас, не вывод о качестве; триггер
  пересмотра — конец триала/рост объёма, и тогда сначала Together (тот же nova-3 в 5× дешевле).
  Результаты — `docs/research/2026-08-06-cloud-stt-smoke-results.md`; дополнение к
  ADR-0005 («Проверка облачных вариантов»). Побочно найден баг — **LS-30**: реконнект
  `DeepgramSTT` обнуляет шкалу времени сегментов, ломает атрибуцию спикера (см. `backlog.md`).
  LS-13 остаётся плановой (self-host GigaAM в посегментном режиме на CPU — не замерено,
  сколько дорожек влезает параллельно).

## Сделано (последнее, LS-30 — шкала времени Deepgram)

- **LS-30 закрыт** (`0511c43`, `067635b`). Корневая причина: `sttStreamStartedAtMs` фиксируется
  один раз на первом аудио-чанке сессии (`websocket/handler.ts:521`), а `startSec` приходит от
  Deepgram и отсчитывается от текущего соединения — после `tryReconnect()` момент реплики
  отскакивал к началу встречи, и `pickSpeakerAt` подписывал её первым говорившим. Доказано
  инструментированным прогоном: `close` → `open`, следом позиция сегмента `9.16 → 0`.
- Починка: `stt/stream-clock.ts` (смещение = реально отправленные в сокет байты на момент
  реконнекта) + `stt/connection-guard.ts` (результаты от неактуального соединения
  отбрасываются). Тесты 111/111, позиции на реальной записи строго монотонны.
- Остаточное ограничение и следующий шаг — в `backlog.md` (LS-30 «граница применимости», LS-31).
- **Важно проверить на живом звонке:** гипотеза, что LS-28 (реплики подписаны не тем
  участником) вызывалась именно этим багом. Возможно, чинить там уже нечего.

## Следующее

- **Перепроверить LS-28** после этого фикса — до того, как браться за браузерный дебаг.
...
- Или новая фича из бэклога (LS-05 упаковка, LS-04 reconnect UX, LS-01/02 платформы; LS-13
  self-host STT — смок закрыт, остаётся замер `transcribe()` GigaAM на CPU; LS-30 — баг шкалы
  времени Deepgram-реконнекта).
...
- **Транскрипт доезжает до кабинета** — разбор обращения клиента `ivanovaa1992@gmail.com`
  ...
  - **LS-23 и LS-24 подтверждены пользователем в браузере** 06.08: склейка даёт одну запись,
    пометка «Пауза» рисуется, Teams-звонки доезжают до кабинета.
  - **LS-25 и LS-27 сделаны**, ждут проверки: правки клиентские, нужна перезагрузка расширения
    из `packages/extension/dist`.
  - **Открыты LS-26** (WS отваливается каждые ~60 с), **LS-28** (реплики привязываются не к тому
    спикеру в Teams) — обе требуют прогона в браузере; **LS-29** — уточнение по паузе.

## Доска задач

Работа ведётся на доске Vikunja — проект **Skribo** (id 5) на https://kanban.skribo.ru; скиллы
`task-board` / `check-tasks` / `watch-board`, правила — в `AGENTS.md` (принципы 4–5).
`docs/backlog.md` остаётся планировочным документом, `LS-NN` — общий ключ.
