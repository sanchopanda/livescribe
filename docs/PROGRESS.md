...
- **LS-06 — Хостинг бэкенда.** Решение зафиксировано (ADR-0002: отдельная ВМ Yandex Cloud,
  свой домен, Caddy TLS/wss, systemd). Ждём от владельца: регистрацию домена и создание ВМ
  (IP + ssh). Параллельно — прод-подготовка кода (конфиг WS-URL, CORS, manifest).
