...
- **LS-12 — email + сброс пароля** (нужен выбор почтового провайдера/SMTP) — начат брейншторм.
- Либо: задеплоить LS-09 (A+B) на сервер (`OPENROUTER_API_KEY`) и проверить анализ/саммари вживую.
- Опц. follow-up LS-14: desktop-нотификация; предзаполнить триггеры именем аккаунта.

## Задача в работе

- Нет (LS-09B закрыт). Коммиты LS-09B в `origin` не запушены.
