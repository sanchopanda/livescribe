...
- **LS-08 sub-plan 4 — Карточка встречи** (`/meetings/:id`: транскрипт по спикерам +
  панель-заглушка анализа; `GET /api/meetings/:id` уже готов). Затем — деплой кабинета на
  `app.skribo.ru` и LS-09 (анализ).
