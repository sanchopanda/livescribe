...
- **Транскрипт доезжает до кабинета** — разбор обращения клиента `ivanovaa1992@gmail.com`
  ...
  - **LS-25 сделан** (свой голос в per-track — Pachca и Meet), не задеплоен: правка чисто
    клиентская, нужна перезагрузка расширения. Проверка в браузере — за пользователем.
  - **Открыт LS-26** (WS отваливается каждые ~60 с) — требует прогона в реальном браузере.
