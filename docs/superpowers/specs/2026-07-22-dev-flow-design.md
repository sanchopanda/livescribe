...
- **`docs/PROGRESS.md`** — курсор состояния. Разделы:
  **Сделано** (последнее), **Следующее** (кандидат), **Задача в работе**
  (id / ветка / PR), **Блокеры**.
- **Источник истины статуса** = эти файлы + git (коммит в `main`). На каждой
...
- **`proceed`** — оркестратор рабочего цикла, **ровно одна задача за вызов**:
  ...
  5. `implement-task` → `test-task` (петля до зелёного) → `review-task`
     (чинить критичное до чистоты) — гейт **до** коммита.
  6. Коммит(ы) по смыслу **прямо в `main`** и пуш (как принято в репо; PR не
     обязателен — второго ревьюера нет). Ветку заводить только при необходимости
     изоляции.
...
- **`implement-task`** (тонкий):
  - Работа **прямо в `main`** (как принято в репо); ветка `feat/<id>-<slug>`
    (латиница, kebab-case) — только когда нужна изоляция.
