...
- [ ] LS-11 — **Нормальный логин в расширении.** Убрать ручной ввод токена: вход
  (email+пароль) прямо в расширении; в идеале — авто-подхват сессии, если пользователь уже
  залогинен в кабинете (кросс-доменные куки — спроектировать). Приоритет по просьбе владельца.
