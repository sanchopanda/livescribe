...
- [ ] LS-25 — **Свой микрофон не попадает в транскрипт (per-track).** MAIN-world хук подписан
  только на `pc.addEventListener('track')` — это событие только для **входящих** дорожек, а
  микрофон уходит через sender. Итог: собственная речь не транскрибируется и не видна даже в
  audio levels. Правка: перехватывать `pc.addTrack` симметрично `ontrack` и заводить свой трек
  участником (имя — из собственной плитки, откат на «Вы»). Второе разрешение на микрофон не
  нужно, страница уже им владеет. Дыра одинаковая у Pachca и Meet.
