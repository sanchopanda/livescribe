...
- [ ] LS-30 — **🐞 Переподключение Deepgram обнуляет шкалу времени сегментов.** Найдено при
  ...
  чинила LS-20 (таймлайн смен спикера ждёт монотонный `startSec`). Воспроизводится на записи
  `recording-f11102c6` через `scripts/stt-smoke` (`npm run stt:smoke -- --provider deepgram`).
  На доске — карточка #51, метка `bug`.
